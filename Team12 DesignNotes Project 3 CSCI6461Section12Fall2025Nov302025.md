...
Usage notes:
- To run the GUI, compile all Java files in `src` and launch with `java SimulatorGUI`.
- The GUI loads `load_file.txt` (assembled output) and sets R0 to the paragraph buffer address for paragraph search programs.
- For paragraph search, `paragraph_search.asm` no longer uses `PFILE` or constants; the GUI runtime loader and R0 initialization are used instead.

Running paragraph search via the GUI (recommended steps):
- **Compile & launch:** `javac *.java` then `java SimulatorGUI` from the `src` folder.
- **IPL:** Click the `IPL` button and select `load_file.txt` produced by the assembler.
- **Set paragraph start:** In the `OCTAL INPUT` box enter `000100` (this is octal address `100` where the paragraph buffer starts in the sample program).
...
Notes:
- If you changed the buffer address in `paragraph_search.asm`, use that octal address in `OCTAL INPUT` instead of `000100`.
- The GUI also attempts to auto-detect probable text buffers after IPL and may pre-fill `OCTAL INPUT` for you.
...
---
## Test Cases and Running with .jar

### Test Cases
- Regression harnesses are provided for major programs, including paragraph search (Program 2).
- Example: `HeadlessRegressionParagraph.java` runs automated tests for paragraph search, verifying both found and not-found cases.
- Other harnesses cover fixed-20, unrolled N, and streaming N programs.

### Running with an Existing .jar
- If you have a compiled `.jar` (e.g., `Simulator.jar`), you can run it via:
	```
	java -jar Simulator.jar
	```
- The GUI will launch, allowing you to load `load_file.txt` and interact with the simulator.
- For headless regression, use:
	```
	java -jar Simulator.jar HeadlessRegressionParagraph
	```
	(or run the harness class directly if not packaged in the jar)

### Updating Paragraph Input
- The paragraph for Program 2 is read from `source.txt` (or fallback `test.txt`) in the workspace root.
- To change the paragraph, edit `source.txt` and re-run the assembler and simulator.
- Ensure the paragraph fits within the buffer size (default: 240 bytes, starting at address 100 in memory).

### Notes
- After updating `source.txt`, reassemble your `.asm` file to generate a new `load_file.txt`.
- The GUI and harnesses will use the updated paragraph on next run.
...
## TRAP services (software interrupts)

Encoding: `TRAP <id>` where `<id>` is an 8‑bit service id encoded in the instruction’s address field.

- 0: Legacy interactive paragraph flow — prints paragraph at `R0` (0‑terminated), prompts, reads word, searches, prints result.
- 1: Print memory — `R0=start`, `R1=len`; prints `len` chars from memory to console.
- 2: Read a word — `R0=dest`; reads until newline; returns `R1=len`.
- 3: Search paragraph — `R0=start`, `R1=len`, `R2=wordStart`, `R3=wordLen`; prints result; returns `R0=sentence#`, `R1=word#` (0,0 if not found).
- 4: Load paragraph from file — `R0=start`, `R1=maxBytes`; loads `source.txt` (fallback `..\source.txt` or `test.txt`) into memory; zero‑terminates if room; returns `R1=actualBytes`.
